...
double kdNode::LB(double*q, statistics& stat)
{
	...
	cur_bandwidth = stat.bandwidth_vec[stat.cur_b_Num];
	...
	if (stat.kernel_type == 0) //Triangular kernel
		LB_value = idList.size()*(1.0 - (1.0 / cur_bandwidth)*ub);
	if (stat.kernel_type == 1) //Epanechnikov kernel
		LB_value = idList.size()*(1.0 - (1.0 / (cur_bandwidth*cur_bandwidth))*ub*ub);
	if (stat.kernel_type == 2) //Quartic kernel
	{
		temp_value = 1.0 - (1.0 / (cur_bandwidth*cur_bandwidth))*ub*ub;
...
double kdNode::UB(double*q, statistics& stat)
{
	...
	if (stat.kernel_type == 0) //Triangular kernel
		UB_value = idList.size()*(1.0 - (1.0 / cur_bandwidth)*lb);
	if (stat.kernel_type == 1) //Epanechnikov kernel
		UB_value = idList.size()*(1.0 - (1.0 / (cur_bandwidth*cur_bandwidth))*lb*lb);
	if (stat.kernel_type == 2) //Quartic kernel
	{
		temp_value = 1.0 - (1.0 / (cur_bandwidth*cur_bandwidth))*lb*lb;
...
void kdTree::RQS(statistics& stat)
{
	...
	stat.out_visual[stat.cur_b_Num][stat.cur_r][stat.cur_c] = density_value;
}

void kdTree::obtain_boundary(statistics& stat)
{
	double*cur_q = stat.queryVector[stat.cur_r*stat.n_col + stat.cur_c];
	stat.query_boundary[0][0] = cur_q[0] - stat.bandwidth_vec[stat.cur_b_Num];
	stat.query_boundary[0][1] = cur_q[0] + stat.bandwidth_vec[stat.cur_b_Num];
	stat.query_boundary[1][0] = cur_q[1] - stat.bandwidth_vec[stat.cur_b_Num];
	stat.query_boundary[1][0] = cur_q[1] + stat.bandwidth_vec[stat.cur_b_Num];
}