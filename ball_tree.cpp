...
double ballTree::RQS_Recur(ballNode*node, statistics& stat)
{
	...
	for (int c = 0; c < (int)node->childVector.size(); c++)
	{
		...
		if (dist_value + childNode->radius <= stat.bandwidth_vec[stat.cur_b_Num])
		{
			density_value += refinement(childNode, stat);
			continue;
		}

		if (dist_value >= stat.bandwidth_vec[stat.cur_b_Num] + childNode->radius)
			continue;
...
void ballTree::RQS(statistics& stat)
{
	...
	if (dist_value + ((ballNode*)rootNode)->radius <= stat.bandwidth_vec[stat.cur_b_Num])
		density_value = refinement(rootNode, stat);
	else if (dist_value >= stat.bandwidth_vec[stat.cur_b_Num] + ((ballNode*)rootNode)->radius)
		density_value = 0;
	else
